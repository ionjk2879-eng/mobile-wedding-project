...
`npm run dev:worker`를 쓰려면 프로젝트 루트에 `.dev.vars` 파일(git에 커밋하지 않음)이 필요합니다:

```
JWT_SECRET=아무-테스트-값
SUPER_ADMIN_EMAIL=슈퍼관리자로-테스트할-이메일
KAKAO_REST_API_KEY=...
KAKAO_CLIENT_SECRET=...
NAVER_CLIENT_ID=...
NAVER_CLIENT_SECRET=...
```
`SUPER_ADMIN_EMAIL`은 `/superadmin` 페이지와 청첩장 결제 활성화(`handleActivate`) 등
슈퍼관리자 전용 기능이 "이 이메일과 로그인 이메일이 일치하는지"로 권한을 판단할 때 씁니다.
