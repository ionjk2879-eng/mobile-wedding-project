          ...
          {pages.map((group, pi) => (
            <div key={pi} className="theme-slide-page">
              <div className="theme-select-grid modern">
                {group.map(t => (
                  <button key={t.key} type="button" className={`theme-chip-v2 ${theme === t.key ? 'active' : ''}`} style={theme === t.key ? { borderColor: t.colors[2] } : {}} onClick={() => updateFields({ theme: t.key, customBgColor: t.colors[0], customAccentColor: t.colors[2], customLabelColor: t.colors[2], customTextColor: t.colors[3] })}>
