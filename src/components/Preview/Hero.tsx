...
//
// (메인화면에는 예전에 hero-fade-in(0.8s opacity+translateY 등장 애니메이션)이
// 있었다. 물결과의 겹침이 튐의 원인인지 Playwright로 재현 테스트한 적이 있는데,
// 그때는 "겹침이 원인이 아니다"로 결론 내렸었다 — 하지만 그 테스트는 실제 사진 없이
// 빈 사진 placeholder 상태에서 인풋값 변경으로 Hero만 리마운트시킨 것이었고(진짜
// 콜드 로드가 아님), CPU를 인위적으로 6배 스로틀링한 것이라 실제 저사양 기기와는
// 조건이 달랐다. 이후 hero-fade-in을 실제로 완전히 제거하자 버벅임이 해소된 게
// 실사용에서 재확인됐다 — 즉 겹침이 실제로는 튐의 원인이었고, 위 테스트 방식(리마운트
// 시뮬레이션 + 인위적 스로틀링 + 사진 없는 상태)이 실제 콜드 로드 상황을 제대로
// 재현하지 못했던 것으로 보인다. hero-fade-in은 이제 완전히 제거됐으므로 이 문단은
// 앞으로 비슷한 "겹침 vs 렌더링 비용" 가설을 다시 테스트할 때 실제 사진을 채운
// 콜드 로드로 검증해야 한다는 참고용으로만 남겨둔다.)
...
// 닿아야 뭉게구름처럼 안 보이고 "물결"로 읽힌다. 레이어 2장 모두 테마 배경색
// (--wedding-bg) 하나만 쓰고, 투명도와 속도만 다르게 줘서 입체감을 낸다 — 뒤쪽은
// 느리고 옅게, 앞쪽은 빠르고 진하게(실제 색 지정은 preview.css 참고). SVG 폭을
// 200%로 두고 정확히 -50%만큼 translateX 하면 뷰박스 절반 단위로 반복되는 패스가
// 이음매 없이 무한 루프된다.
